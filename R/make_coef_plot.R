...
#' make_coef_plot Make a plot of treatment effects with confidence intervals against a dotted line at zero
#'
...
#' @param return_ci_table If TRUE, returns the table of confidence intervals underlying the plot
...
make_coef_plot <- function(model_list = list(sample_model_1, sample_model_2, sample_model_3),
  ...
  if(length(treat_vars) == 1){
    treat_vars <- rep(treat_vars, length(models))
  }

  # Create an empty data frame where you'll place everything
  plotting_df <- dplyr::tibble(term = rep(NA, length(model_list)),
  ...
  for (i in 1:length(model_list)) {
    ...
    coef_plot <- ggplot2::ggplot(plotting_df) + ggplot2::geom_point(ggplot2::aes(x = model, y = estimate,  color = factor(model)), stat = "identity")+
      ggplot2::geom_errorbar(data = plotting_df, ggplot2::aes(x = model, ymin =lower,ymax = upper, color = factor(model)), width = .75) +
